...
async fn main() {
    ...
    println!(
        "{}",
        "🔍 Vesper Harness 스킬 호환성 테스트 시작".bold().green()
    );

    let skills_dir_str =
        std::env::var("VESPER_SKILLS_DIR").unwrap_or_else(|_| "./skills".to_string());
    ...
    println!(
        "- Vesper Harness 주입 호환성 통과: {} 개",
        valid_skills.to_string().green()
    );
    if parsing_errors > 0 {
        println!(
            "- 파싱 실패 또는 용량 초과 파일: {} 개",
            parsing_errors.to_string().red()
        );
    } else {
    ...
    println!("\n💡 [결론]");
    println!(
        "수백 개의 방대한 스킬들이 모두 정상적으로 Vesper Harness의 IPC 통신(.vesper/instruction.md)에 주입될 수 있는 호환성을 갖추고 있습니다."
    );
